...
class GamePickServiceTest {
    ...
    @BeforeEach
    void setUp() {
        gamePickService = new GamePickService(gamePickRepository, authService, CLOCK);
    }
    ...
    void submitCreatesPickForAuthenticatedUser() {
        ...
        var response = gamePickService.submit("Bearer token", 778899L, new GamePickRequest(111L, "Los Angeles Dodgers"));
    ...
    void submitUpdatesExistingPickForSameUserAndGame() {
        ...
        var response = gamePickService.submit("Bearer token", 778899L, new GamePickRequest(147L, "New York Yankees"));
